...
public class CommunityAttachmentService {
    ...
    private static final String SAFE_FILE_MIME = "application/octet-stream";
    ...
    public CommunityAttachmentDto uploadFile(String ownerEmail, MultipartFile file) {
        ...
        CommunityAttachmentEntity entity = attachmentRepository.save(CommunityAttachmentEntity.builder()
                ...
                .originalName(stored.originalName()).contentType(SAFE_FILE_MIME).sizeBytes(stored.sizeBytes())
                .storageKey(stored.storageKey()).sortOrder(0).build());
        return toDto(entity);
    }
    ...
    /**
     * Keeps storage and repository access behind the application boundary.
     * The caller must already be authenticated; post visibility remains owned
     * by the existing community read policy and is intentionally not invented
     * here.
     */
    @Transactional(readOnly = true)
    public DownloadAttachment loadForAuthenticatedUser(String userEmail, String attachmentId) {
        if (userEmail == null || userEmail.isBlank()) {
            throw ApiException.forbidden("COMMUNITY_ATTACHMENT_AUTH_REQUIRED", "인증이 필요합니다.");
        }
        CommunityAttachmentEntity entity = attachmentRepository.findById(attachmentId)
                .orElseThrow(() -> ApiException.notFound("COMMUNITY_ATTACHMENT_NOT_FOUND", "첨부를 찾을 수 없습니다."));
        if (entity.getPostId() == null && !userEmail.equals(entity.getOwnerEmail())) {
            throw ApiException.notFound("COMMUNITY_ATTACHMENT_NOT_FOUND", "첨부를 찾을 수 없습니다.");
        }
        if (entity.getStorageKey() == null) {
            throw ApiException.notFound("COMMUNITY_ATTACHMENT_NOT_DOWNLOADABLE", "다운로드할 수 없는 첨부입니다.");
        }
        boolean inline = "IMAGE".equals(entity.getAttachmentType()) && ALLOWED_IMAGE_MIME.contains(entity.getContentType());
        String contentType = inline ? entity.getContentType() : SAFE_FILE_MIME;
        return new DownloadAttachment(storage.load(entity.getStorageKey()), contentType, entity.getOriginalName(), inline);
    }
    ...
    public record DownloadAttachment(Resource resource, String contentType, String originalName, boolean inline) {
    }
}
